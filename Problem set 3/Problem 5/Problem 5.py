...
def run_mcmc(fun,pars,true_y,Ninv,corr_mat,nstep=150):
    chain=[]
    chivec=np.zeros(nstep)
    chol=np.linalg.cholesky(corr_mat)
    L=np.linalg.pinv(chol).transpose()
    ...
    for i in range(nstep):
        ...
        pred=fun(pars_trial)
        resid=true_y-pred[2:len(true_y)+2]
        chi_trial=resid.transpose()@Ninv@resid+((pars_trial[3]-0.0544)/0.0073)**2 #updated chi
...
chain,chivec,reject=run_mcmc(get_spectrum,pars,wmap[:,1],Ninv,cov_mat)
